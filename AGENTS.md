...
- **API**: Axios client (`src/api/client.ts`) — Bearer token on requests; response 401 interceptor queues concurrent retries and refreshes via `/v1/auth/refresh`. Refresh uses `withCredentials: true` (cookie-based). `PUBLIC_AUTH_PATHS` exempts login/register/refresh from retry. `register` in `src/api/auth.ts` sends `email`, `password`, `first_name`, `last_name`, optional `role` (`'buyer' | 'jastiper'`).
- **Auth**: JWT in `localStorage('access_token')`; auto-refresh via `setTimeout` at `expires_in - 60` seconds. `AuthSync` component in `App.tsx` wires `setForceLogoutHandler` to Redux `logout` action. If no handler registered, `triggerForceLogout` fallback clears localStorage + redirects to `/login`.
